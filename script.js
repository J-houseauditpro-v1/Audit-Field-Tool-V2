...
var PHOTO_CATEGORIES = [
  { id: 'hvac',       label: 'HVAC / Condenser', short: 'HVAC'  },
  { id: 'duct',       label: 'Duct Sealing',      short: 'DUCT'  },
  { id: 'air',        label: 'Air Sealing',       short: 'AIR'   },
  { id: 'water',      label: 'Water Heater',      short: 'WTR'   },
  { id: 'insulation', label: 'Insulation/Attic',  short: 'INSUL' },
  { id: 'exterior',   label: 'Exterior/Doors',    short: 'EXT'   },
  { id: 'general',    label: 'General',           short: 'GEN'   }
];

var PRESET_NOTES = {
  hvac: [
    'Condenser needs to be cleaned, no cover removal necessary.',
    'Evaporator is dirty, needs cleaning.',
    'Filter is dirty, no replacement on hand.',
    'Qualifies for smart thermostat — gave customer the QR code.'
  ],
  duct: [
    'Ductwork appears to be in excellent condition. No issues found.',
    'Duct needs sealed at ___.'
  ],
  air: [
    'PP needs foam sealed.',
    'PP already sealed with silicone.',
    'PP to be sealed.'
  ],
  water: [
    'Water heater, electric storage, in unconditioned space. Temp set to ___. No adjustment made.',
    'Electric storage water heater, ___ gallon.'
  ],
  insulation: [
    "Attic insulation defect area, ___ (___ sq ft). Redistribute existing insulation by hand to fill this defect area.",
    'Attic has average of ___" blown insulation throughout.'
  ],
  exterior: [
    'Corner patch needed.',
    'Door needs bolster near the top on the jam side.',
    'Door needs weatherstripping.'
  ],
  general: [
    'No issues found.'
  ]
    ...
    files.forEach(function(f) {
      ...
      r.onload = function(ev) {
        compressImage(ev.target.result, function(compressed) {
          ...
          savePhotoToDB({ id: id, auditId: auditId, dataUrl: compressed, note: '', category: '', ts: ts }, function(ok) {
            if (ok) {
              S.photos.push({ id: id, auditId: auditId, note: '', category: '', ts: ts });
              addedIds.push(id);
...
function renderCategoryChips(selectedId) {
  ...
  wrap.innerHTML = PHOTO_CATEGORIES.map(function(c) {
    return '<button type="button" class="chip' + (c.id === selectedId ? ' active' : '') + '" data-cat="' + c.id + '">' + c.label + '</button>';
  }).join('');
  wrap.querySelectorAll('.chip').forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      var newCat = (current && current.category === catId) ? '' : catId; // tap active chip again to clear
...
function renderPresetNotes(categoryId) {
  ...
  var label = document.getElementById('modal-preset-label');
  if (!wrap) return;
  var presets = PRESET_NOTES[categoryId] || [];
  if (!presets.length) {
    wrap.innerHTML = '';
  ...
  wrap.innerHTML = presets.map(function(text, i) {
    return '<button type="button" class="preset-chip" data-preset-idx="' + i + '">' + text + '</button>';
  }).join('');
  wrap.querySelectorAll('.preset-chip').forEach(function(btn) {
    btn.addEventListener('click', function() {
      insertPresetNote(presets[Number(btn.dataset.presetIdx)]);
    });
  });
}
...
function insertPresetNote(text) {
  var noteEl = document.getElementById('modal-note');
  if (!noteEl) return;
  noteEl.value = noteEl.value ? (noteEl.value.replace(/\s+$/, '') + ' ' + text) : text;
  scheduleNoteAutosave();
  noteEl.focus();
  noteEl.selectionStart = noteEl.selectionEnd = noteEl.value.length;
}
...
function initModal() {
  ...
  document.getElementById('modal-note').addEventListener('input', scheduleNoteAutosave);
      ...
      S.photos.forEach(function(photo, index) {
        getPhotoFromDB(photo.id, function(record) {
          loadedPhotos[index] = {
            ...
            category: photo.category || (record ? record.category : '') || '',
            ts: photo.ts
          };
          loadCount++;
